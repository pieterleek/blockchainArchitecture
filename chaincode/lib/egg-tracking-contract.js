...
// External entity's server (e.g. https://1c2d39be.ngrok.io)
//const EXTERNAL_API_SERVER = 'https://4b6a-145-15-244-234.ngrok-free.app';
const EXTERNAL_API_SERVER = '** YOUR_EXTERNAL_SERVER_HERE **';

//version of ngrok version with out .
const NGROK_INSTALLED_VERSION = "3.3.0";
...
class EggTrackingContract extends Contract {
  ...
  async packEggs(ctx, farmerId, packingTimestamp, quantity) {
    ...
    // get external organisation stamp
    let stamp = await this.getFarmerStamp(farmerId,packingTimestamp); 

    console.log('@packEggs - stamp=' + stamp);

    if(!stamp ) {
      throw new Error(`Farmer ${farmerId} is not allowed to pack eggs`);
    }

    // add stamp to eggbox object
    eggBox.stamp = stamp;
  ...
  /**
   * Getting the product stamp from the external entity
   */
  async getFarmerStamp(id,timestamp) {
    try {
      console.log('@debug getFarmerStamp was called');
      let json;
      if(NGROK_INSTALLED_VERSION.match(/\d+/) >= 330 ){
        json = await axios.get(`${EXTERNAL_API_SERVER}/farmer/${id}?packingTimestamp=${timestamp}`,{ headers:{ 
        "ngrok-skip-browser-warning": "69420" }});
      }
      else
      {
        json = await axios(`${EXTERNAL_API_SERVER}/farmer/${id}?packingTimestamp=${timestamp}`);
      }
      console.log('@debug json',json.data)
      console.log('@debug stamp = ' + json.data.stamp)
      return json.data.stamp;
    } catch(error) {
